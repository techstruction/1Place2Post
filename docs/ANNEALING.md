...
### MISTAKE 23: Prisma client must be regenerated after schema changes — TypeScript errors in new services

**What happened:** After Phase 13a added `THREADS`/`TELEGRAM` to the Platform enum, `workspaceId_platform_platformId` as the unique constraint, and `onboardingCompletedAt`/`userRole` to the User model, the Prisma generated client was not regenerated. When Phase 13b added `ThreadsService`, `TelegramService`, etc. referencing these new schema elements, the TypeScript compile produced 26 errors — `TELEGRAM` and `THREADS` not on Platform enum, `workspaceId_platform_platformId` not on `SocialAccountWhereUniqueInput`, `userRole` not on `UserSelect`.

**Why it's subtle:** Prisma's generated client lives in `node_modules/.prisma/client/`. It reflects the schema at the time `prisma generate` was last run, not the current `schema.prisma` file. If you pull schema changes from git and skip `npx prisma generate`, the TypeScript types are stale even though the runtime migration is applied.

**Fix:** `npx prisma generate` clears all 26 errors immediately.

**Rule:** After any `git pull` that includes Prisma schema changes, always run `npx prisma generate` before writing code that uses new schema elements. Add it to the project setup checklist. The symptom is TypeScript errors claiming enum values or fields don't exist even though they're clearly in `schema.prisma`.

---

### MISTAKE 24: New NestJS service modules must inject PrismaService, not instantiate PrismaClient directly

**What happened:** The plan template for new platform services used `private prisma = new PrismaClient()` (direct instantiation). This works at runtime but bypasses the shared `PrismaService` singleton managed by `PrismaModule`. Using a `new PrismaClient()` creates a second DB connection pool per service, which is wasteful and breaks connection pooling.

**What we did:** Services were updated to use constructor injection — `constructor(private prisma: PrismaService)` — and modules were updated to import `PrismaModule`.

**Rule:** Never use `new PrismaClient()` inside a NestJS service. Always inject `PrismaService` via constructor and import `PrismaModule` in the module. This ensures a single connection pool across all services.
...
@Module({ imports: [PrismaModule], providers: [MyService] })
export class MyModule {}

// ❌ Wrong
@Injectable()
export class MyService {
  private prisma = new PrismaClient(); // second connection pool
}
```

---

### MISTAKE 25: OAuth state-passing pattern — token in query param is a security anti-pattern

**What happened:** The current frontend modal pattern for OAuth-initiating platforms passes the JWT like this:
```typescript
window.location.href = `${API}/social/instagram/auth?token=${token}&workspaceId=${workspaceId}`;
```
The `?token=` query param approach means the JWT is logged in server access logs, browser history, and referrer headers. The `JwtAuthGuard` on the `GET /auth` endpoints needs to be updated to accept the token from the Authorization header — which requires a redirect flow instead of a direct href.

**Why it's deferred:** This was a known trade-off in Phase 13b. Fixing it requires either a server-side session handoff or a client-side redirect through an intermediate page that sets the Authorization header. This is low-risk in the short term (the token is used for one redirect, not stored) but is a Phase 15 security hardening item.

**Rule:** OAuth initiation endpoints that need user identity should not accept JWTs as query parameters. The correct pattern is a short-lived server-side session token or a pre-auth code. Filed as Phase 15 security work.

---

### MISTAKE 26: stale `.next/types` cache causes phantom TypeScript errors after page deletions

**What happened:** After Phase 13a deleted `app/dashboard/team/` (replaced by `workspace/`), the `.next/types/validator.ts` file still contained a reference to the old page path. Running `npx tsc --noEmit` in `apps/web` showed one error: `Cannot find module '../../app/dashboard/team/page.js'`.

**Why it's subtle:** `.next/types/` is auto-generated by the Next.js dev server or build process. It doesn't regenerate when you delete pages — only when you run `next build` or start the dev server. If you run `tsc --noEmit` without regenerating `.next/types/`, you see stale errors.

**Fix:** `rm -rf apps/web/.next` before running `tsc --noEmit` on the frontend. This forces a clean state.

**Rule:** When TypeScript reports errors about missing page modules in `.next/types/`, the `.next` directory is stale. Run `rm -rf apps/web/.next` and re-check. This is always safe — `.next` is a build artifact, never committed, always regenerated.
